...
export class DisciplineInfoComponent implements OnInit {
  ...
  private refreshTables(): void {
    this.disciplineService.getCycleCommissions().subscribe(response => {
      ...
      const cycleCommission = this.cycleCommissionId
        ? this.cycleCommissions.find(_ => _.id === this.cycleCommissionId)
        : this.cycleCommissions[0];
